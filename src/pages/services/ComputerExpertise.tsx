...
const faq = [
  {
    question: "Ноутбук вышел из строя через 3 месяца после покупки. Продавец говорит — это я сломал. Как доказать обратное?",
    answer: "Бремя доказывания в течение гарантийного срока лежит на продавце (ст. 18 ЗоЗПП). Именно он обязан провести экспертизу за свой счёт. Если она показывает «механическое повреждение», вы вправе оспорить это заключение, заказав независимую экспертизу. При подтверждении производственного брака суд обяжет продавца вернуть деньги и оплатить ваши расходы."
  },
  {
    question: "Можно ли вернуть ноутбук с матрицей, которая треснула сама по себе?",
    answer: "Да, если экспертиза установит, что матрица имела скрытый производственный дефект (расслоение, микротрещина от завода). Это встречается чаще, чем кажется. Внешнее повреждение не всегда означает вину покупателя — эксперт может определить природу разрушения по характеру излома и следам деформации."
  },
  {
    question: "Что считается «существенным недостатком» для технически сложного товара?",
    answer: "Существенный недостаток — это неустранимый дефект, дефект, требующий несоразмерных затрат на устранение, проявляющийся неоднократно или после ремонта снова. Наличие существенного недостатка даёт право на возврат денег даже по истечении 15 дней с момента покупки."
  },
  {
    question: "Гарантийный срок истёк, но компьютер сломался. Есть ли шансы на возврат?",
    answer: "Да. По ст. 19 ЗоЗПП покупатель вправе предъявить претензии в течение 2 лет со дня покупки, даже если гарантия уже закончилась. Для этого нужно доказать, что дефект производственный. Именно для таких случаев и делается независимая экспертиза."
  },
];
...
export default function ComputerExpertise() {
  return (
    <ServiceLayout
      ...
      description="Независимое заключение о производственных дефектах ноутбуков, системных блоков и мониторов. Для возврата, гарантийного ремонта и судебных споров."
      faq={faq}
    >
